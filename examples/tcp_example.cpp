...
int main() {
  ...
  /* 切包回调 */
  auto server_split_cb = [](const TcpServer::Session &,
  ...
  /* 服务端帧回调 */
  auto server_fragment_cb = [](const TcpServer::Session &session,
  ...
  /* 创建一个 TCP 客户端 */
  auto client_addr = InetAddr::make_ipv4("127.0.0.1", 0).value();
  auto client = TcpClient::make_tcp_client(*loop, client_addr);

  /* 客户端拆包逻辑：同服务端 */
  auto client_split_cb = [](const std::shared_ptr<TcpClient> &,
                            std::string_view slice) -> ssize_t {
    /* 成功拆包返回包长，失败返回0 */
    if (slice.size() < 4)
      return 0;
  ...
  /* 客户端帧回调 */
  auto client_fragment_cb = [](const std::shared_ptr<TcpClient> &session,
                               std::vector<uint8_t> fragment) {
