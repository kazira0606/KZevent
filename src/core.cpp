...
namespace kzevent::core {
  ...
  Loop::Loop() {
    ...
    if (const auto ret = epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, io_wake_fd_, &io_wake_ev); ret < 0) {
      sys_error::fatal();
    }
  ...
  Loop::~Loop() {
    /* 禁止在executor线程析构 */
    if (std::this_thread::get_id() == io_thread_.get_id() || std::this_thread::get_id() == heavy_thread_.get_id()) {
  ...
  void Loop::start() {
    ...
    if (auto expected{ExeState::Stopped}; !exe_state_.compare_exchange_strong(expected, ExeState::Running)) {
  ...
  void Loop::stop() {
    if (auto expected{ExeState::Running}; !exe_state_.compare_exchange_strong(expected, ExeState::Stopping)) {
    ...
    auto task = [this, fd] {
      if (const auto it = events_.find(fd); it == events_.end()) {
        ...
        if (it->second.in_epoll == true) {
          if (const auto ret = epoll_ctl(epoll_fd_, EPOLL_CTL_DEL, fd, nullptr); ret < 0 && errno != ENOENT) {
            sys_error::error();
          }
        }
        /* 取消注册 */
  ...
  void Loop::enable_fd(const int32_t fd, const EventType types, const EventMode modes, CallBack cb) {
  ...
  void Loop::wake_heavy_up() {
    heavy_wake_cv_.notify_one();
  }
  ...
  void Loop::io_executor() {
    ...
    while (exe_state_.load() == ExeState::Running) {
      ...
      for (auto it = evs.begin(); it != (evs.begin() + ev_count); it += 1) {
        ...
        /* 执行回调 */
        if (const auto event_it = events_.find(it->data.fd); event_it != events_.end()) {
    ...
    if (auto expected{ExeState::Stopping}; !exe_state_.compare_exchange_strong(expected, ExeState::Stopped)) {
    ...
    while (exe_state_.load() == ExeState::Running) {
      std::unique_lock wait_lock(heavy_queue_mtx_);
      {
        heavy_wake_cv_.wait(wait_lock, [this] {
          return !heavy_buffer_queue_.empty() || exe_state_.load() != ExeState::Running;
        });
  ...
  void FdHandler::update_event(LifeChecker life_checker, const EventType types,
    ...
    auto wrapper_cb = [life_checker = std::move(life_checker), cb = std::move(cb)](const EventType event_types) {
